defmodule Assignment.CoindataCoordinator do
  ...
  def balance(checkIfAllPairsAreRunning \\ true) do
    ...
    nodes = filter_out_reporter([node() | Node.list()])
  ...
  def handle_call(:retrieve_all_worker_pids, _from, state) do
    {:reply, retrieve_all_registry_workerkeys(), state}
  end
  ...
  def filter_out_reporter(list) do
    if(length(list) < 1) do
      ...
      [head | tail] = list
      headstring = Atom.to_string(head)
      if (String.starts_with?(headstring, "reporter")) do
        List.flatten([filter_out_reporter(tail)])
      else
        List.flatten([head, filter_out_reporter(tail)])
      end
    end
  end

  def handle_continue(:start_children, state) do
    length = length(filter_out_reporter(Node.list()))
    if(length < 1) do
      if(length < 1) do
